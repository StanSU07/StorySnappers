    ...
    public class Dialogue
    {
        ...
        public void FillStoryWithPlayers(List<string> playerNames) 
        {
            foreach (KeyValuePair<string, Node> node in nodes)
            {
                ...
                {
                    node.Value.text = node.Value.text.Replace($"${i}$", playerNames[i]);

                    for (int j = 0; j < node.Value.responses.Count; j++)
                    {
                        string responseText = node.Value.responses[j].displayText;
                        responseText = responseText.Replace($"${i}$", playerNames[i]);
                        node.Value.responses[j] = new Response(responseText, node.Value.responses[j].destinationNode);
                    }
                }
            }
        }
